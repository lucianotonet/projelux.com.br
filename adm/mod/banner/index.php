<script type="text/javascript">
  function excluir(id){
      if(window.confirm('Excluir este banner?')){
          window.location.href='menu.php?sub=banner&acao=deleta_item&id='+id;
      }
  }
</script>
<?php
...
<h5>Banner &raquo; <a href="#" class="active" >Lista de Banners</a></h5>

<?php
    $sql_total="SELECT * FROM banners";
    $sql="SELECT * FROM banners ORDER BY ban_nome LIMIT ".$primeiro_registro.", ".$num_pag." ";
    ...
    $pg_atual = "menu.php?sub=banners&pagina=";
        ...
        <?php
            echo "<tr>";
                ...
                echo "<td class='action' style='width:5%'>";
                    echo "<a href='menu.php?sub=banner&acao=edita_item&id=".$row['ban_id']."' class='edit'><i class='fa fa-pencil fa-fw'></i></a>";
                echo "</td>";
                echo "<td class='action' style='width:5%'>";
                    echo "<a onclick='excluir(".$row['ban_id'].")' href='#' class='delete'><i class='fa fa-remove fa-fw'></i></a>";
                echo "</td>";
            echo "</tr>";
        ?>
    </table>
